...
def invariance_test_ellipsoid(epsi, l_, *, conf_lev=0.95, fit=0, r=2,
    ...
    rho = np.array([stats.pearsonr(epsi[:-k]-meancov_sp(epsi[:-k])[0],
    ...
    alpha = 1-conf_lev
    z_alpha_half = stats.norm.ppf(1-alpha/2) / np.sqrt(epsi.shape[0])
    ...
    if plot_test:
        ...
        if fit == 1:  # normal
            sns.distplot(xxx, hist_kws=hist_kws, kde=False, fit=stats.norm, ax=ax)
            ...
            sns.distplot(xxx, hist_kws=hist_kws, fit_kws=fit_kws,
                         kde=False, fit=stats.expon, ax=ax)
            ...
            k = np.arange(x.max() + 1)
            mlest = x.mean()
            plt.plot(k, stats.poisson.pmf(k, mlest), 'o', linestyle='-', lw=1,
        ...
        if fit == 1:
            sns.distplot(yyy, hist_kws=hist_kws, kde=False, fit=stats.norm,
                         vertical=True, ax=ax)
            plt.xticks(fontsize=14)
            plt.yticks(fontsize=14)
        elif fit == 2 and sum(epsi < 0) == 0:
            sns.distplot(yyy, hist_kws=hist_kws, fit_kws=fit_kws,
                         kde=False, fit=stats.expon, vertical=True, ax=ax)
            plt.xticks(fontsize=14)
            plt.yticks(fontsize=14)
            ...
            plt.plot(stats.poisson.pmf(k, mlest), k, 'o', linestyle='-',
