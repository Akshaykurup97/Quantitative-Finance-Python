...
def plot_ellipse(m, s2, *, r=1, n_=1000, display_ellipse=True, plot_axes=False,
    ...
    e, lam = pca_cov(s2)
    Diag_sqrtlam = np.diagflat(np.sqrt(np.maximum(lam, 0)))
    ...
    if display_ellipse:
        ...
        if plot_axes:
            # principal axes
            plt.plot([m[0]-r*np.sqrt(lam[0])*e[0, 0],
                      m[0]+r*np.sqrt(lam[0])*e[0, 0]],
                     [m[1]-r*np.sqrt(lam[0])*e[1, 0],
                      m[1]+r*np.sqrt(lam[0])*e[1, 0]], color=color,
                     lw=line_width)

            plt.plot([m[0]-r*np.sqrt(lam[1])*e[0, 1],
                      m[0]+r*np.sqrt(lam[1])*e[0, 1]],
                     [m[1]-r*np.sqrt(lam[1])*e[1, 1],
                      m[1]+r*np.sqrt(lam[1])*e[1, 1]], color=color,
                     lw=line_width)
            plt.axis('equal')
